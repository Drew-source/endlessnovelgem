...
from narrative import handle_narrative_turn
...
class GameApp:
    ...
    def process_player_turn(self, input_text: str):
        """Process a player turn in the background"""
        try:
            # Increment turn counter
            self.turn_count += 1
            
            # Update game state with player input
            self.game_state["last_player_action"] = input_text
            
            # Check if we're in dialogue mode (implement dialogue handling here)
            if self.game_state.get("dialogue_active", False):
                # Handle dialogue turn (not fully implemented in this example)
                self.result_queue.put(("status", "Dialogue not fully implemented in this example"))
                processed_text = "Dialogue system would process your response here."
                
                # For now, just append to narrative
                current_narrative = self.narrative_display.value.replace("*Processing...*", processed_text)
                self.result_queue.put(("narrative", current_narrative))
            
            else:
                # Handle narrative turn
                self.result_queue.put(("status", "Processing narrative turn..."))
                
                # Append User Message & Truncate History
                user_message = {"role": "user", "content": input_text}
                self.conversation_history.append(user_message)
                
                # Call narrative handler (this example is simplified)
                ...
                processed_text, initial_resp_obj, tool_results, final_resp_obj, stop_flag = handle_claude_response(
                ...
                # Update game state's narrative context
                self.game_state["narrative_context_summary"] = (
                    f"{self.game_state.get('narrative_context_summary', '')}\n\n{processed_text}"
                )
                ...
                if not stop_flag:
                    self.result_queue.put(("status", "Generating visuals..."))
                    gemini_prompt = construct_gemini_prompt(
                        narrative_text=processed_text,
                        game_state=self.game_state,
                        placeholder_template=self.prompt_templates.get("gemini_placeholder_template", "")
                    )
                    placeholder_output = call_gemini_api(self.gemini_client, self.gemini_model_name, gemini_prompt)
                    self.result_queue.put(("placeholders", placeholder_output))
